...
TEST_F(MockVendorTests, DoubleInheritance)
{
    // Test setup
    ...
    auto obj_e_mock = std::make_shared<ObjEMock>();

    // Add expectations on the mock
    EXPECT_CALL(*obj_e_mock, obj_e_func());
    EXPECT_CALL(*obj_e_mock, obj_a_func());
    EXPECT_CALL(*obj_e_mock, obj_d_func());

    // Create a mock vendor for objects of ObjE.
    ObjEMockVendor obj_e_mv;
